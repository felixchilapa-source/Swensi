           ...
           <div className="relative w-full max-w-[420px] bg-white dark:bg-slate-900 rounded-[40px] p-8 shadow-2xl border border-white/10 max-h-[90vh] overflow-y-auto no-scrollbar">
              ...
              <div className="space-y-6">
                <div className="p-4 bg-purple-500/5 border border-purple-500/20 rounded-2xl">
                   ...
                   <p className="text-[10px] font-bold text-slate-500">This mission will automatically generate a Nakonde Council Order (Levy: ZMW {(selectedCategory.basePrice * 0.05).toFixed(2)}).</p>
                </div>
                ...
                <div key={b.id} className="bg-white dark:bg-slate-900 rounded-[40px] border border-slate-100 dark:border-white/5 shadow-2xl overflow-hidden mb-8">
                  <div className="p-8">
                    ...
                    {councilOrder && (
                      <div className="p-5 bg-purple-600/5 dark:bg-purple-600/10 rounded-3xl mb-6 border border-purple-600/20">
                         ...
                         <div className="flex justify-between items-end">
                            <div>
                               <p className="text-[11px] font-black text-secondary dark:text-white uppercase italic tracking-tighter">{councilOrder.id}</p>
                               <p className="text-[7px] font-bold text-slate-400 uppercase mt-0.5">Levy: ZMW {councilOrder.levyAmount.toFixed(2)}</p>
                            </div>
                            <div className="w-8 h-8 rounded-lg bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm text-purple-600"><i className="fa-solid fa-stamp"></i></div>
                         </div>
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-white/5 rounded-3xl border border-slate-100 dark:border-white/5">
                       <div className="flex flex-col">
                          <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Escrow Commit</span>
                          <span className="text-sm font-black dark:text-white">ZMW {b.price.toFixed(2)}</span>
                       </div>
                       <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm"><i className="fa-solid fa-fingerprint text-emerald-600"></i></div>
                    </div>
                  </div>
                </div>
              );
        ...
        {activeTab === 'account' && (
          <div className="animate-fade-in space-y-6">
            <div className="bg-white dark:bg-slate-900 rounded-[45px] p-10 border border-slate-100 dark:border-white/5 shadow-2xl text-center relative overflow-hidden">
               ...
               <button onClick={logout} className="w-full text-red-500 font-black py-4 text-[9px] uppercase tracking-[0.3em] hover:bg-red-500/5 rounded-2xl transition-colors mt-12">Deauthorize Terminal</button>
            </div>
          </div>
        )}
      </div>
