...
const NewsTicker: React.FC = () => {
  ...
  return (
    <div className="w-full bg-slate-900 py-2.5 overflow-hidden border-y border-white/5 relative h-9 flex items-center shadow-2xl">
      <div className="absolute left-0 top-0 bottom-0 px-4 bg-slate-900/95 backdrop-blur-md z-20 flex items-center border-r border-white/5 shadow-[10px_0_20px_rgba(0,0,0,0.5)]">
        <span className="text-[8px] font-black uppercase text-blue-500 tracking-[0.3em] italic">Intelligence</span>
      </div>
      <div className="flex animate-[ticker_40s_linear_infinite] whitespace-nowrap items-center pl-10">
        {news.concat(news).map((item, idx) => (
          <div key={idx} className="flex items-center gap-6 px-6 border-r border-white/10">
             <div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></div>
             <span className="text-[10px] font-black text-slate-300 uppercase tracking-tight italic">{item}</span>
          </div>
        ))}
      </div>
    </div>
