            ...
            {requests.map(req => (
              <div key={req.id} className="bg-white dark:bg-slate-900 p-6 rounded-[32px] shadow-xl border border-slate-100 dark:border-white/5">
                 <div className="flex justify-between items-start mb-4">
                   ...
                   <span className="text-[8px] font-black bg-purple-600/10 text-purple-600 px-3 py-1 rounded-full uppercase">Pending</span>
                 </div>
                 <div className="flex gap-2 mt-4">
                    <input 
                      ...
                      className="w-24 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl px-4 text-xs font-black text-slate-800 dark:text-white outline-none focus:border-purple-600" 
                    />
                    <button 
                      onClick={() => handleCheckIn(req.id)} 
                      disabled={roomsCount === 0}
                      className={`flex-1 font-black py-4 rounded-2xl text-[9px] uppercase shadow-lg italic transition-all ${roomsCount > 0 ? 'bg-purple-600 text-white' : 'bg-slate-200 text-slate-400'}`}
                    >
                      {roomsCount > 0 ? 'Confirm Check-In' : 'No Rooms Available'}
                    </button>
                 </div>
              </div>
            ))}
             ...
             {guests.map(g => (
               <div key={g.id} className="bg-white dark:bg-slate-900 p-5 rounded-[30px] border border-slate-100 dark:border-white/5 shadow-md flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-[8px] font-bold text-purple-600 uppercase mt-1 tracking-widest">Room {g.roomNumber}</p>
                  </div>
